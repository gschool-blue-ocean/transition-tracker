        ...
        <div id={style.cohort-container}>
            <div id={style.cohort-nav} className={style.mainContainer}>
                ...
                <div className={style.list}>
                    ...
                    <div onClick={handleClickedAllBtn} id={style.all-cohort-btn-div} className={`${style.listOfCohorts} ${style.activeCohortTab}`}>
                        <button id={style.all-cohorts-btn} >
                            All
                        </button>
                    </div>

                    {
                        cohortsToMap.map(cohort => {
                            ...
                            return (
                                <div id={cohort.cohort_id} onClick={handleCohortClicked} className={style.listOfCohorts }><p>{cohort.cohort_name}</p></div>
                            )

                        })
                    }
                    <button onClick={setNewCohortModalIsOpenToTrue} id={style.add-cohort-btn}>
                        +
                    </button>
                </div>
                                    ...
                                    return (
                                        <div className={style.test-cohort} key={cohort.cohort_id}>
                                            <div className={style.cardHeader}>
                                                <div className={style.cardName} id={cohort.cohort_id} onClick={handleCohortClicked}>{cohort.cohort_name}</div>
                                            ...
                                            <div className={style.listOfNames}>
                                                {
                                                    allUsersData.map(user => {
                                                        ...
                                                        if (user.cohort_id == cohort.cohort_id) {
                                                            return <div className={style.nameInRow}>

                                                                <SPETStag userETS={user.ets_date} className={style.ets-tag} />
                                                                ...
                                                                <div id={`${JSON.stringify(user)}`} className={style.name-div} onClick={handleStudentNameClick} > {user.first} {user.last} </div> <div className={style.color-code}></div>
                                                                {/*^^^^^^^^^^^^^^^^^^^^^^^ */}
                                                            </div>
                                                        }
                                                    })
                                                }
                                                <div id={style.numberOfStudents}>{allUsersData.filter(user => user.cohort_id == cohort.cohort_id).length} Students</div>
                                            </div>
            ...
            <Modal isOpen={modalIsOpen} portalClassName={style.modal}>
                <button className={style.x} onClick={setModalIsOpenToFalse}>X</button>
                <EditCohortPage selectedID={style.currentCohort} />
            </Modal>
            <Modal isOpen={newCohortModalIsOpen} portalClassName={style.newCohortModal}>
                <button className={style.x} onClick={setNewCohortModalIsOpenToFalse}>X</button>
                <NewCohortModal />
            </Modal>
        </div>
