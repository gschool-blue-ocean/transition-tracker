...
export default function SPTasks({ activeStudent }) {
   ...
   if (loading) {
      ...
      return (
         <div className={style.SDash-Tasks}>
            <h4 id="StuTasks-Header">Tasks</h4>
            <button
               className={style.StuTasks-CreateBtn}
               onClick={() => {
                  setCreateTask(true);
                  openModal();
               }}
            ...
            <>
               {studentTasks.length === 0 ? (
                  ...
                  studentTasks.map((task) => {
                     return (
                        <div className={style.StuTasks-Card}>
                           <div
                              className={style.StuTasks-Title}
                              id={task.task_id}
                              key={task.task_id}
                              onClick={() => {
                                 viewTask(task);
                           ...
                           <button
                              className={style.StuTasks-Buttons}
                              onClick={() => {
                                 setEditTask(true);
                                 openModal();
                                 setSelectedTask(task);
